...
export class LoginComponent implements OnInit {
  ...
  public async login() {
    ...
    try {
      ...
      if (resposta?.user) this.router.navigate(['/painel']);
    } catch (error: any) {
    ...
    try {
      ...
      if (resultado === 'enviar') {
        ...
        this.toastrService.success(
          `Um email com instruções de recuperação foi enviado com sucesso para: ${this.emailRecuperacao?.value}.`,
      ...
      if (error != '0' && error != '1' && error != 'fechar')
        this.toastrService.error(
          'Houve um erro ao tentar enviar o email! Verifique o endereço.',
